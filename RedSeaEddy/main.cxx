...
{
	vtkSmartPointer<vtkStructuredGridReader> mean_reader_a1 = 
    vtkSmartPointer<vtkStructuredGridReader>::New();

	mean_reader_a1->SetFileName(argv[1]);
	mean_reader_a1->Update();

	vtkSmartPointer<vtkStructuredGrid> mesh_a1 = 
    vtkSmartPointer<vtkStructuredGrid>::New();
	mesh_a1 = mean_reader_a1->GetOutput();
	
	int num_pts = mesh_a1->GetNumberOfPoints();
		
	vtkSmartPointer<vtkStructuredGridReader> dev_reader_a1 = 
    vtkSmartPointer<vtkStructuredGridReader>::New();
	dev_reader_a1->SetFileName(argv[2]);
	dev_reader_a1->Update();

	vtkSmartPointer<vtkStructuredGrid> std_mesh_a1 = 
    vtkSmartPointer<vtkStructuredGrid>::New();
	std_mesh_a1 = dev_reader_a1->GetOutput();

	cout << "Has array : " << mesh_a1->GetPointData()->GetArrayName(0) << endl;
	cout << "Has array : " << std_mesh_a1->GetPointData()->GetArrayName(0) << endl;
	
	vtkSmartPointer<vtkStructuredGridReader> mean_reader_a2 = 
    vtkSmartPointer<vtkStructuredGridReader>::New();

	mean_reader_a2->SetFileName(argv[3]);
	mean_reader_a2->Update();

	vtkSmartPointer<vtkStructuredGrid> mesh_a2 = 
    vtkSmartPointer<vtkStructuredGrid>::New();
	mesh_a2 = mean_reader_a2->GetOutput();
	
	vtkSmartPointer<vtkStructuredGridReader> dev_reader_a2 = 
    vtkSmartPointer<vtkStructuredGridReader>::New();
	dev_reader_a2->SetFileName(argv[4]);
	dev_reader_a2->Update();

	vtkSmartPointer<vtkStructuredGrid> std_mesh_a2 = 
    vtkSmartPointer<vtkStructuredGrid>::New();
	std_mesh_a2 = dev_reader_a2->GetOutput();

	cout << "Has array : " << mesh_a2->GetPointData()->GetArrayName(0) << endl;
	cout << "Has array : " << std_mesh_a2->GetPointData()->GetArrayName(0) << endl;

	vtkAbstractArray* a1 = mesh_a1->GetPointData()->GetAbstractArray(argv[10]);
	vtkAbstractArray* a2 = std_mesh_a1->GetPointData()->GetAbstractArray(argv[11]);
	vtkAbstractArray* a3 = mesh_a2->GetPointData()->GetAbstractArray(argv[12]);
	vtkAbstractArray* a4 = std_mesh_a2->GetPointData()->GetAbstractArray(argv[13]);

	vtkFloatArray* mean_a1 = vtkFloatArray::SafeDownCast(a1);
	vtkFloatArray* dev_a1 = vtkFloatArray::SafeDownCast(a2);
	vtkFloatArray* mean_a2 = vtkFloatArray::SafeDownCast(a3);
	vtkFloatArray* dev_a2 = vtkFloatArray::SafeDownCast(a4);

	int dims[3];

  mesh_a1->GetDimensions(dims);
...
// Output the range of each attribute 
	...
	double range[2]; 
	mean_a1->GetRange(range);
	cout << "Att1 range " << range[0] << " to " << range[1] << endl;
	
	float att1_min = atof(argv[6]);
	float att1_max = atof(argv[7]);
	
	cout << "Interval start: " << att1_min << endl;
	cout << "Interval end: " << att1_max << endl;
	
	float att2_min = atof(argv[8]);
	float att2_max = atof(argv[9]);
...
//	int *binary_image = (int*)malloc(sizeof(int)*num_pts);
	float *minvals_a1 = (float*)malloc(sizeof(float)*num_pts);
	float *maxvals_a1 = (float*)malloc(sizeof(float)*num_pts);
	float *minvals_a2 = (float*)malloc(sizeof(float)*num_pts);
	float *maxvals_a2 = (float*)malloc(sizeof(float)*num_pts);
	...
	{
		double c1 = mean_a1->GetTuple1(n);
		double c2 = mean_a2->GetTuple1(n);
		if(c1 > att1_min && c1 < att1_max && c1 < att2_min && c2 < att2_max) 
		{
...
// Confidence interval = 50. Z = 0.67449
	...
	for(int n = 0; n < num_pts; n++)
  {
    ...
    double c2 = mean_a2->GetTuple1(n);
		...
		minvals_a1[n] = c1 - (Z*dev_a1->GetTuple1(n));
		maxvals_a1[n] = c1 + (Z*dev_a1->GetTuple1(n));
		
		minvals_a2[n] = c2 - (Z*dev_a2->GetTuple1(n));
		maxvals_a2[n] = c2 + (Z*dev_a2->GetTuple1(n));
	}
		
	
	for(int n = 0; n < num_pts; n++)
  {
		if((minvals_a1[n] <= att1_max && maxvals_a1[n] >= att1_min) && (minvals_a2[n] <= att2_max && maxvals_a2[n] >= att2_min)) 
    {
	   	distArray_50->InsertNextTuple1(0.0);
//      binary_image[n] = 0;
      cells_50++;
    }
    else
    {
			distArray_50->InsertNextTuple1(1.0);
//      binary_image[n] = 1;
    }
  }
/*
...
// Confidence interval = 95. Z = 1.95996
	...
	for(int n = 0; n < num_pts; n++)
  {
    ...
    {
//      binary_image[n] = 0;
			distArray_95->InsertNextTuple1(0.0);
      cells_95++;
    }
    else
    {
			distArray_95->InsertNextTuple1(1.0);
//      binary_image[n] = 1;
    }
  }
/*
